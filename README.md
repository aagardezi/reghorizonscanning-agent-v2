...
### Setup & Installation
1. **Airlock / Proxy Configuration (If applicable)**:
   If you are running in an environment with Airlock/corporate proxies enabled and experience package installation or update failures, you must initialize `gpkg` first:
   ```bash
   gpkg setup
   ```
   Ensure the exported proxy variables generated by `gpkg setup` are active in your terminal environment.
